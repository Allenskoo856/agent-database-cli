---
name: database-cli
description: 使用本地 database-cli 安全操作已配置的数据库。适用于列出数据库连接、测试连接、执行 SQL/Redis/MongoDB 命令、查询表/列/集合/keys 元信息、管理本地连接 daemon，以及验证只读模式和命令黑名单的场景。
---

# database-cli 使用说明

`database-cli` 是一个基于本地配置的多数据库命令行工具，用于让 AI 或用户安全地操作数据库。
...
```bash
DATABASE_CLI_CONFIG=/path/to/config.json node dist/cli.js list
```
...
```bash
DATABASE_CLI_CONFIG=/path/to/config.json
```
...
```bash
database-cli exec --db "<databaseName>" --command "<command>"
```
...
```bash
database-cli meta --db "<databaseName>" --type tables
database-cli meta --db "<databaseName>" --type columns --table users
database-cli meta --db "<databaseName>" --type collections
database-cli meta --db "<databaseName>" --type keys --pattern "user:*"
```
...
```bash
database-cli reset --db "<databaseName>"
```
