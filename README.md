...
| --- | --- | --- | --- | --- |
| MySQL | `mysql` | Rust 原生驱动 `mysql_async` | 暂不支持切换 | `readonly`、`blacklist`、`keepAliveSeconds` |
| PostgreSQL | `postgres` | Rust 原生驱动 `tokio-postgres` | 暂不支持切换 | `readonly`、`blacklist`、`keepAliveSeconds` |
| Redis 单机 | `redis` | Rust 原生驱动 `redis` | 仅配置 `url` | `readonly`、`blacklist`、`keepAliveSeconds` |
| Redis 集群 | `redis` | Rust 原生驱动 `redis` | 同时配置 `url` 和 `redisCluster.nodes` | `readonly`、`blacklist`、`keepAliveSeconds` |
...
| MongoDB | `mongodb` | Rust 原生驱动 `mongodb` | 暂不支持切换；可配 `database` 指定默认库 | `readonly`、`blacklist`、`keepAliveSeconds` |
...
项目已新增 Rust CLI 骨架，命令名保持 `agent-database-cli` 语义，当前 Rust 二进制名为 `agent-database-cli`：
